import { ImportDataDto } from '@dexfolio/api/app/import/import-data.dto';
...
import { DataProviderInterface } from '@dexfolio/api/services/data-provider/interfaces/data-provider.interface';
...
import type { Granularity, UserWithSettings } from '@dexfolio/common/types';
...
export class DataProviderService implements OnModuleInit {
  ...
  public async getDataSources(): Promise<DataSource[]> {
    ...
    const dexfolioApiKey = await this.propertyService.getByKey<string>(
      PROPERTY_API_KEY_dexfolio
    );

    if (dexfolioApiKey) {
      dataSources.push('DEXFOLIO');
    }
  ...
  public async getQuotes({
    ...
    if (numberOfItemsInCache) {
      Logger.debug(
        `Fetched ${numberOfItemsInCache} quote${numberOfItemsInCache > 1 ? 's' : ''
      ...
      for (
        ...
        promises.push(
          promise.then(async (result) => {
            ...
            Logger.debug(
              `Fetched ${symbolsChunk.length} quote${symbolsChunk.length > 1 ? 's' : ''
              } from ${dataSource} in ${(
