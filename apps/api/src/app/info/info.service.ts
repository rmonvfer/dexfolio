...
import { InfoItem, Statistics } from '@dexfolio/common/interfaces';
...
export class InfoService {
  ...
  private async countDockerHubPulls(): Promise<number> {
    try {
      const { pull_count } = (await fetch(
        'https://hub.docker.com/v2/repositories/dexfolio/dexfolio',
        {
  ...
  private async countGitHubContributors(): Promise<number> {
    try {
      const body = await fetch('https://github.com/dexfolio/dexfolio', {
        signal: AbortSignal.timeout(
          this.configurationService.get('REQUEST_TIMEOUT')
        )
      }).then((res) => res.text());
      ...
      return extractNumberFromString({
        value: $(
          'a[href="/dexfolio/dexfolio/graphs/contributors"] .Counter'
        ).text()
      });
    } catch (error) {
  ...
  private async countGitHubStargazers(): Promise<number> {
    try {
      const { stargazers_count } = (await fetch(
        'https://api.github.com/repos/dexfolio/dexfolio',
        {
          headers: { 'User-Agent': 'request' },
          signal: AbortSignal.timeout(
            this.configurationService.get('REQUEST_TIMEOUT')
