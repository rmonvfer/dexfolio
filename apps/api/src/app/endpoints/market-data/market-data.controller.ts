import { AdminService } from '@dexfolio/api/app/admin/admin.service';
...
import { TransformDataSourceInRequestInterceptor } from '@dexfolio/api/interceptors/transform-data-source-in-request/transform-data-source-in-request.interceptor';
import { TransformDataSourceInResponseInterceptor } from '@dexfolio/api/interceptors/transform-data-source-in-response/transform-data-source-in-response.interceptor';
...
import {
  dexfolioFearAndGreedIndexDataSourceCryptocurrencies,
  dexfolioFearAndGreedIndexDataSourceStocks,
  dexfolioFearAndGreedIndexSymbolCryptocurrencies,
  dexfolioFearAndGreedIndexSymbolStocks
} from '@dexfolio/common/config';
import { UpdateBulkMarketDataDto } from '@dexfolio/common/dtos';
import { getCurrencyFromSymbol, isCurrency } from '@dexfolio/common/helper';
import {
  ...
  public async getMarketDataOfMarkets(
    ...
    const [
      ...
      this.symbolService.get({
        ...
        dataGatheringItem: {
          dataSource: dexfolioFearAndGreedIndexDataSourceCryptocurrencies,
          symbol: dexfolioFearAndGreedIndexSymbolCryptocurrencies
        }
      }),
      this.symbolService.get({
        includeHistoricalData,
        dataGatheringItem: {
          ...
          symbol: dexfolioFearAndGreedIndexSymbolStocks
        }
      })
    ]);
