...
import { SubscriptionService } from '@dexfolio/api/app/subscription/subscription.service';
...
import { getRandomString } from '@dexfolio/api/helper/string.helper';
...
import { TagService } from '@dexfolio/api/services/tag/tag.service';
import {
  DEFAULT_CURRENCY,
...
export class UserService {
  ...
  public async user(
    ...
    const user: UserWithSettings = {
      ...
      dataProviderdexfolioDailyRequests:
        analytics?.dataProviderdexfolioDailyRequests
    };
    ...
    if (this.configurationService.get('ENABLE_FEATURE_SUBSCRIPTION')) {
      ...
      if (user.subscription?.type === 'Basic') {
        ...
        if (!hasRole(user, Role.DEMO)) {
          ...
          currentPermissions.push(permissions.enableDataProviderDexfolio);
  ...
  public async resetAnalytics() {
    return this.prismaService.analytics.updateMany({
      data: {
        dataProviderdexfolioDailyRequests: 0
      },
      where: {
        updatedAt: {
