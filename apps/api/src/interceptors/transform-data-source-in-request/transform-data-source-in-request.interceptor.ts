...
import { decodeDataSource } from '@dexfolio/common/helper';
  ...
  public intercept(
    ...
    if (this.configurationService.get('ENABLE_FEATURE_SUBSCRIPTION')) {
      if (request.body?.activities) {
        const dataSourcedexfolioDataProvider = this.configurationService.get(
        ...
        request.body.activities = request.body.activities.map((activity) => {
          if (DataSource[activity.dataSource]) {
            if (
              activity.dataSource === 'DEXFOLIO' &&
              dataSourcedexfolioDataProvider
            ) {
              return {
                ...activity,
                dataSource: dataSourcedexfolioDataProvider
              };
            } else {
              return activity;
            }
