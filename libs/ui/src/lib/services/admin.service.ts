...
import { GF_ENVIRONMENT } from '@dexfolio/ui/environment';
...
export class AdminService {
  ...
  public fetchDexfolioDataProviderStatus(aApiKey: string) {
    const headers = new HttpHeaders({
      [HEADER_KEY_SKIP_INTERCEPTOR]: 'true',
      [HEADER_KEY_TOKEN]: `Api-Key ${aApiKey}`
    });

    return this.http.get<DataProviderDexfolioStatusResponse>(
      `${this.environment.production ? 'https://dexfol.io' : ''}/api/v2/data-providers/dexfolio/status`,
      { headers }
    );
  }
